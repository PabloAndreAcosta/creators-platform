  ...
  try {
    ...
    if (user) {
      const { data } = await supabase
        .from("bookings")
        .select("id, scheduled_at, status, notes, amount_paid, stripe_payment_id, is_free, booking_type, creator_id, listings(title, category, price, listing_type, image_url, event_date, event_time, event_location)")
