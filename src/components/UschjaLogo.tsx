"use client";

import { useEffect, useId, useState } from "react";

export type LogoLage = "mork" | "ljus" | "system";
const NYCKEL = "uschja-logo-lage";

export function getLogoLage(): LogoLage {
  if (typeof window === "undefined") return "system";
  const v = localStorage.getItem(NYCKEL);
  return v === "mork" || v === "ljus" ? v : "system";
}
export function setLogoLage(lage: LogoLage) {
  localStorage.setItem(NYCKEL, lage);
  window.dispatchEvent(new Event("uschja-logo-lage"));
}

function arMork(lage: LogoLage): boolean {
  if (lage === "mork") return true;
  if (lage === "ljus") return false;
  // "system": always use the dark branded tile. It's the recognizable Usha mark
  // (same as the app icon and login) and stays visible on BOTH app backgrounds.
  // The previous behaviour followed the device colour scheme, which rendered a
  // near-white tile on the light app background (the default) — so the logo
  // blended into the page and looked like an empty square.
  return true;
}

function Monogram({ dag }: { dag: boolean }) {
  // Unique per-instance gradient IDs. Several UschjaLogo instances render on the
  // same page (header + the display:none desktop sidebar, which appears FIRST in
  // the DOM). With shared IDs the browser resolved `url(#…)` to the sidebar's
  // gradients — but gradients inside a display:none subtree don't paint, so the
  // visible header logo's gradient fills vanished, leaving only the solid-stroke
  // border (the "empty square" bug). useId() gives each instance its own IDs.
  const raw = useId();
  const uid = raw.replace(/:/g, ""); // ":" is awkward inside url(#…) on old Safari
  const p = `${uid}p`, g = `${uid}g`, s = `${uid}s`, d = `${uid}d`;
  const platta = dag ? ["#FFFBEF", "#F7ECD2"] : ["#292524", "#0C0A09"];
  const guld = dag ? ["#E3BC4B", "#C9A22F", "#8F6F1F"] : ["#F5D061", "#D4AF37", "#A8842B"];
  const dot = dag ? ["#FEF3C7", "#E3BC4B", "#B08C28"] : ["#FEF3C7", "#F5D061", "#D4AF37"];
  const kant = dag ? "#C9A22F" : "#D4AF37";
  return (
    <svg viewBox="80 80 240 240" aria-hidden="true" style={{ width: "100%", height: "100%", display: "block" }}>
      <defs>
        <linearGradient id={p} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={platta[0]} /><stop offset="100%" stopColor={platta[1]} /></linearGradient>
        <linearGradient id={g} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor={guld[0]} /><stop offset="55%" stopColor={guld[1]} /><stop offset="100%" stopColor={guld[2]} /></linearGradient>
        <linearGradient id={s} x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#FCD34D" /><stop offset="55%" stopColor="#FB923C" /><stop offset="100%" stopColor="#C2410C" /></linearGradient>
        <radialGradient id={d} cx="50%" cy="38%" r="70%"><stop offset="0%" stopColor={dot[0]} /><stop offset="60%" stopColor={dot[1]} /><stop offset="100%" stopColor={dot[2]} /></radialGradient>
      </defs>
      <rect x="80" y="80" width="240" height="240" rx="64" fill={`url(#${p})`} />
      <rect x="86" y="86" width="228" height="228" rx="58" fill="none" stroke={kant} strokeWidth="2" opacity=".6" />
      <path d="M128 156 L128 222 Q128 258 172 258 Q216 258 216 222 L216 156 L195 156 L195 220 Q195 239 172 239 Q149 239 149 220 L149 156 Z" fill={`url(#${g})`} />
      <rect x="242" y="142" width="26" height="80" rx="13" fill={`url(#${s})`} />
      <circle cx="255" cy="245" r="14" fill={`url(#${d})`} />
    </svg>
  );
}

/** Usha Platform-logga som följer inställningen uschja-logo-lage. */
export default function UschjaLogo({ size = 64 }: { size?: number }) {
  // Deterministic initial value for SSR/first paint = the dark branded tile,
  // which is the default ("system") result; the real setting is read on mount
  // (localStorage is client-only). Starting dark avoids a light→dark flash.
  const [mork, setMork] = useState(true);

  useEffect(() => {
    const uppdatera = () => setMork(arMork(getLogoLage()));
    uppdatera();
    const mq = window.matchMedia("(prefers-color-scheme: dark)");
    mq.addEventListener("change", uppdatera);
    window.addEventListener("uschja-logo-lage", uppdatera);
    window.addEventListener("storage", uppdatera); // synk mellan flikar
    return () => {
      mq.removeEventListener("change", uppdatera);
      window.removeEventListener("uschja-logo-lage", uppdatera);
      window.removeEventListener("storage", uppdatera);
    };
  }, []);

  return (
    <span role="img" aria-label="Usha Platform" style={{ display: "inline-block", width: size, height: size }}>
      <Monogram dag={!mork} />
    </span>
  );
}
